/**
 * CREDITS: https://l.devwtf.in/cloudboxbot
 */
...
async function onMessage(message) {
  try {
    ...
    } else if (message.text.startsWith("/mykey")) {
      return await myCode(message.chat.id, message);
...
async function myCode(chatId, message) {
  const userCode = chatId.toString(); // Use the chat ID as the user code

  await sendInlineButtons(
    chatId,
    escapeMarkdown(`Your verification code is: ${userCode}\n\nClick on the "Verify Code" button below and use the official coudbox verification bot to get verified.`),
    [
      [
        {
          text: "Verify Code",
          url: `https://t.me/CLOUDBOX2_OFFICIAL_BOT`,
        },
        {
          text: "Back to Menu",
